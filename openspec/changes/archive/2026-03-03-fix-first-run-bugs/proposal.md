...
During the first manual run of a PAM agent (note-taker example), several issues were discovered that prevent agents from running correctly:
...
3. Install instructions don't mention `pam run`
4. Claude Code runs OOBE setup wizard in the container
5. MCP proxy returns 404 on all endpoints (missing PAM_PROXY_TOKEN env var + token not baked into settings.json)
...
- Always include `PAM_PROXY_TOKEN` in mcp-proxy environment
...
- Update install "Next steps" to show `pam run` as primary command
...
- `docker-compose-generation`: PAM_PROXY_TOKEN always in mcp-proxy environment
- `pam-install-command`: Token generated before materialization, passed to materializers, updated instructions
...
- **Existing behavior**: `pam run` now uses two-phase approach instead of single `up -d`
