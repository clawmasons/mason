...
- [x] 1.1 Create `tests/integration/mcp-proxy.sh` — shell script that builds pam, runs install from example dir, starts mcp-proxy via docker compose, sends MCP protocol requests via curl, and asserts correct responses
...
- [x] 2.1 Run the integration test end-to-end and fix any issues with pam install, proxy config, or docker-compose generation
