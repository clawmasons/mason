...
- [x] 1.1 In `tests/integration/forge-proxy.test.ts`: change `APP_NAME` from `"@example/app-filesystem"` to `"@clawforge/app-filesystem"`
- [x] 1.2 In `tests/integration/mcp-proxy.sh`: rename `EXAMPLE_DIR` variable to `WORKSPACE_DIR`, point at `$PROJECT_ROOT/forge-core`
- [x] 1.3 In `tests/integration/mcp-proxy.sh`: change agent name from `@example/agent-note-taker` to `@clawforge/agent-note-taker`
...
- [x] 2.1 Replace `@example/*` package names in CLI examples with `@clawforge/*`
