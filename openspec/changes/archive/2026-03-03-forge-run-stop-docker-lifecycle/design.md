...
After `forge install <agent>`, a scaffolded directory exists at `.forge/agents/<agent-short-name>/` containing `docker-compose.yml`, `.env`, `mcp-proxy/config.json`, and runtime workspace directories. Users currently must manually invoke `docker compose -f <path>/docker-compose.yml up` to start the agent stack. The `forge run` and `forge stop` commands wrap this into the forge CLI.

The install command already writes the output directory path as `.forge/agents/<short-name>/` by default (or a custom `--output-dir`). The run/stop commands need to locate this directory, validate prerequisites, and delegate to `docker compose`.

## Goals / Non-Goals

**Goals:**
- `forge run <agent>` starts the full Docker Compose stack (proxy + all runtimes)
- `forge run <agent> --runtime=X` starts only the proxy and the specified runtime service
- `forge stop <agent>` tears down the Docker Compose stack
...
The run/stop commands locate the agent's scaffolded directory at `.forge/agents/<short-name>/` relative to the workspace root (current working directory). This matches the install command's default output. An `--output-dir` option is supported for custom locations.
...
Commands delegate to `docker compose` (v2 CLI) using Node.js `child_process.execSync` (or `spawn` for streaming output). The compose file path is passed via `-f`. This keeps forge as a thin orchestration layer.
...
`forge stop` runs `docker compose down` in the agent directory. No flags beyond the agent name are needed. The `--remove-volumes` flag is not exposed to avoid accidental data loss — users can run docker compose directly for advanced teardown.
