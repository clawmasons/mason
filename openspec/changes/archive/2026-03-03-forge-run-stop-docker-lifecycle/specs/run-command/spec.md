...
### Requirement: forge run starts the Docker Compose stack
...
- **WHEN** `forge run my-agent` is executed with valid configuration
...
- **WHEN** `forge run my-agent` is executed and Docker Compose returns a non-zero exit code
