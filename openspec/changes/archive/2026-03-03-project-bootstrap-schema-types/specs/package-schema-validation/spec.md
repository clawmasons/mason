...
The system SHALL provide a `parseForgeField(input: unknown)` function that uses Zod's discriminated union on the `type` field to parse any valid forge field and return a precisely typed result. Invalid inputs SHALL produce actionable Zod error messages.
