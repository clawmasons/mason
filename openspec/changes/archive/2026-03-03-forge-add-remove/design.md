...
The forge CLI has eight commands implemented (init, build, install, validate, list, permissions, run, stop) but lacks `add` and `remove` — the two commands that manage individual package dependencies. Currently, packages must be manually placed in workspace directories or installed via raw `npm install`. There is no npm delegation or forge-field validation on install, and no dependent-checking on removal.

All existing commands follow the Commander.js registration pattern: a `register<X>Command(program)` function in `src/cli/commands/<x>.ts`, registered in `src/cli/commands/index.ts`. Commands call `discoverPackages()` for workspace scanning and use `parseForgeField()` from `src/schemas/` for validation. Error handling uses try/catch with `console.error` + `process.exit(1)`.
...
**Goals:**
- `forge add <pkg>` delegates to `npm install`, then validates the installed package has a valid `forge` field
- `forge remove <pkg>` checks for forge-level dependents before delegating to `npm uninstall`
...
- No automatic rebuilding of agent scaffolds after add/remove — users run `forge install` separately
...
### 2. Post-install validation for `forge add`

After `npm install` succeeds, read the installed package's `package.json` from `node_modules/<pkg>/` and validate its `forge` field with `parseForgeField()`. If validation fails, run `npm uninstall` to clean up, then exit with error.
...
- Accept any package and validate later: Breaks the "consistent graph" invariant — users would have non-forge packages in their workspace.

### 3. Dependent checking for `forge remove` via discoverPackages + graph scan

Before removal, call `discoverPackages()` to get all workspace packages, then scan their `forge` fields for references to the target package. Check:
...
Both commands accept variadic args (`[npmArgs...]`) that are forwarded directly to the underlying npm command. This lets users pass `--save-dev`, `--legacy-peer-deps`, etc. without forge needing to know about them.
...
- **Dependent check is local only**: `forge remove` only checks the local workspace, not published packages that might depend on the target. This is consistent with npm's behavior — it's the local graph we care about.
