...
PAM lacks a working example that users can reference to understand the packaging system. Users need a self-contained directory they can copy, inspect, and run `pam install` against — demonstrating all 5 component types (app, skill, task, role, agent) and the "install from files" workflow without npm registry involvement.
...
- **README.md** with component hierarchy overview, quick start commands, directory structure, and explanation of what `pam install` generates.

## Capabilities

### New Capabilities
- `example-workspace`: A self-contained example workspace demonstrating all PAM component types with a note-taker agent theme
...
- **Validation**: The example must pass all 4 validation checks in `pam validate`
