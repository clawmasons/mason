...
The forge pipeline can resolve agent dependency graphs, validate them, and generate mcp-proxy configs with toolFilters — but it cannot yet produce runtime-specific workspaces. The Claude Code materializer is the first runtime materializer, translating the abstract resolved agent into a directory that Claude Code natively understands: MCP settings, slash commands, role documentation, and skill files. Without this, `forge install` cannot produce a deployable workspace.
...
- Implement the Claude Code materializer that generates:
  - `.claude/settings.json` — single forge-proxy MCP server entry with auth
