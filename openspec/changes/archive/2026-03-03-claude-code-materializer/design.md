...
The forge pipeline currently handles: schema validation → package discovery → dependency resolution → graph validation → toolFilter computation → proxy config generation. The next step is materializing runtime-specific workspaces from the resolved agent graph.
...
- Generate `.claude/settings.json` with a single forge-proxy MCP server entry
...
- `forge install` orchestration (separate change)
...
`materializeWorkspace()` returns a `MaterializationResult` — a map of relative paths to string content. The caller (future `forge install`) handles writing to disk. This keeps materializers pure and testable without filesystem mocking.
...
- **[Skill artifacts are paths, not content]** → The materializer can list expected skill paths but cannot copy actual files without reading source packages. Mitigation: `forge install` will handle the actual file copying; the materializer provides the manifest. For now, tests validate the path structure.
- **[Task prompt content not embedded]** → Task prompts are file paths (e.g., `./prompts/triage.md`). The materializer includes a placeholder reference. Mitigation: `forge install` reads prompt files and substitutes content.
