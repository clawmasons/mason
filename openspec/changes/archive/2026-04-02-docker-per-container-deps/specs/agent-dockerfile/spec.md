...
- The Dockerfile SHALL NOT COPY `node_modules/` or set `ENV PATH` to include `node_modules/.bin`
- The mason user SHALL be created with configurable UID/GID via `ARG HOST_UID`/`ARG HOST_GID` BEFORE any COPY statements
- All COPY statements for application files SHALL use `--chown=mason:mason` to avoid a separate `chown -R` layer
...
#### Scenario: No node_modules in agent container
- **WHEN** `generateAgentDockerfile()` is called
- **THEN** the output Dockerfile SHALL NOT contain `COPY node_modules/`
- **AND** SHALL NOT contain `ENV PATH` referencing `node_modules/.bin`

#### Scenario: COPY uses --chown to avoid duplicate layers
- **WHEN** `generateAgentDockerfile()` is called with workspace files to copy
- **THEN** the mason user creation block SHALL appear BEFORE any COPY of application files
