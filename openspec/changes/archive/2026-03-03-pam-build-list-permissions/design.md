# Design: pam build, list, and permissions Commands
...
**Goals:**
- Implement `pam build <agent>` that writes `pam.lock.json` to the workspace
- Implement `pam list` that shows a tree of all discovered agents and their dependency trees
- Implement `pam permissions <agent>` that displays the permission matrix and toolFilter
...
`pam build <agent>` writes `pam.lock.json` to the current working directory (or `--output <path>`). It does NOT write to the scaffolded agent directory — that's `pam install`'s job. Build is a lightweight graph-lock operation.

**Alternative:** Write to `.pam/agents/<name>/pam.lock.json`. Rejected because build should be usable without a prior install, and the lock file is a CI artifact.

### 2. List scans workspace without requiring an agent argument

`pam list` discovers all packages, finds all agent-typed packages, and resolves each one. No argument required. If `--agent <name>` is provided, shows only that agent's tree.
...
`pam permissions <agent>` shows two views:
...
- **Large workspaces:** `pam list` resolves all agents, which could be slow with many packages. Acceptable for now — optimize later if needed.
