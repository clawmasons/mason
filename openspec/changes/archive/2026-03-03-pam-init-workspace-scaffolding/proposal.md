...
pam has validated schemas for all five package types but no CLI entry point or commands. Developers cannot interact with pam yet. The `pam init` command is the foundational user touchpoint — it creates the monorepo workspace structure that all subsequent commands (add, build, install, run) operate within. Without it, there's no way to start building agent components.

## What Changes

- Add Commander.js as a runtime dependency for CLI argument parsing
- Create `bin/pam.js` CLI entry point registered in package.json `bin` field
- Implement `pam init` command that scaffolds the full workspace structure:
  - Root `package.json` with npm workspaces config (`apps/*`, `tasks/*`, `skills/*`, `roles/*`, `agents/*`)
  - `.pam/config.json` with workspace-level pam configuration defaults
  - `.pam/.env.example` template for credential bindings
  - Type-organized directories: `apps/`, `tasks/`, `skills/`, `roles/`, `agents/`
- Add `.gitignore` to scaffolded workspace (node_modules, .env, dist)
- Handle idempotency: warn if workspace already exists (detected by `.pam/` directory presence)
...
- `workspace-init`: The `pam init` command that scaffolds a monorepo workspace with all required directories, config files, and npm workspace configuration
...
- **package.json:** Adds `bin` field pointing to `bin/pam.js`, adds commander dependency
...
- **Build output:** `dist/cli/` directory with compiled CLI code, `bin/pam.js` shebang wrapper
