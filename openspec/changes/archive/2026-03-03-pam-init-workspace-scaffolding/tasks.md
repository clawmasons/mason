...
- [x] 1.2 Create `bin/pam.js` shebang wrapper that imports and runs `dist/cli/index.js`
- [x] 1.3 Add `"bin": { "pam": "./bin/pam.js" }` to package.json
...
- [x] 3.1 Implement workspace detection: check for `.pam/` directory, warn and exit if found
- [x] 3.2 Implement directory scaffolding: create `apps/`, `tasks/`, `skills/`, `roles/`, `agents/`, `.pam/`
...
- [x] 3.4 Implement `.pam/config.json` generation with `{ "version": "0.1.0" }`
- [x] 3.5 Implement `.pam/.env.example` generation with commented credential placeholders
- [x] 3.6 Implement `.gitignore` generation with `node_modules/`, `.env`, `dist/`, `.pam/.env`. Skip if `.gitignore` already exists.
