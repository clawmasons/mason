...
The pam pipeline has all the building blocks implemented as pure functions:
...
**Goals:**
- Wire all existing pipeline stages into a single `pam install <agent>` command
- Write all generated artifacts to a structured output directory
- Generate a random `PAM_PROXY_TOKEN` and inject it into the `.env` file
...
### 2. Output directory: `.pam/agents/{short-name}/` by default

The scaffolded directory lives inside the pam workspace's `.pam/agents/` directory. This keeps generated artifacts separate from source packages and follows the pattern set by `pam init` creating `.pam/`. The `--output-dir` flag allows override for custom layouts.

### 3. Token generation: `crypto.randomBytes(32).toString('hex')`

A 64-character hex string generated via Node's built-in `crypto.randomBytes`. No external dependency needed. The token is written directly into the `.env` file's `PAM_PROXY_TOKEN=` line, replacing the empty placeholder from `generateEnvTemplate()`.
