...
pam can resolve the full dependency graph from an agent package, but has no way to check that the graph is semantically valid — that role permissions actually cover the tools their tasks need, that allowed tools actually exist on the referenced apps, and that all task skill requirements are satisfied. Without `pam validate`, there's no governance gate for CI/CD pipelines and no way for developers to catch permission gaps before attempting to install or run an agent.
...
- Implement the `pam validate <agent>` CLI command that discovers packages, resolves the agent graph, runs validation, and outputs structured results with an appropriate exit code
