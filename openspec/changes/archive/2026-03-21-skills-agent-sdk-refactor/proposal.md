...
- **Use `role.sources` for explicit source resolution** — mason-dialect roles (in `.mason/roles/`) auto-detect as `agentDialect: "mason"`, but may need skills/tasks from agent-specific directories like `.claude/`. The existing `sources` field in the role schema (e.g., `sources: [".claude"]`) is now used to explicitly declare where skills/tasks come from. The `getSourceTaskConfig` and `getSourceSkillConfig` functions check `role.sources` first, resolving each entry (e.g., `.claude` → `claude-code-agent` package) via `getDialectByDirectory` before falling back to the auto-detected dialect.
- **Add validation warnings for missing skills/tasks** — `resolveSkillContent` and `resolveTaskContent` now emit `console.warn` when a referenced skill or task is not found in the source directory, making misconfigured roles fail loudly instead of silently producing hollow output.
- **Fix supervisor path to resolve skills** — `docker-generator.ts` now calls `resolveSkillContent` alongside `resolveTaskContent` in the supervisor Dockerfile generation path, ensuring skills are populated for all materialization paths (not just `materializeForAgent`).
...
- **packages/shared/src/types.ts** — `ResolvedSkill` gains an optional `contentMap` field for inline artifact data.
- **packages/cli/src/materializer/docker-generator.ts** — imports and calls `resolveSkillContent` alongside `resolveTaskContent` in the supervisor path.
- **.mason/roles/lead/ROLE.md** and **.mason/roles/developer/ROLE.md** — add `sources: [".claude"]` to frontmatter so skill/task resolution finds files in `.claude/` directory.
