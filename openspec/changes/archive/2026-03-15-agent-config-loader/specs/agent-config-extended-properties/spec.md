...
Each agent entry in `.mason/config.json` SHALL support three optional launch-profile properties in addition to the existing `package` field: `home` (string), `mode` (one of `"terminal"`, `"acp"`, `"bash"`), and `role` (string). These properties define per-agent invocation defaults for the current project.

#### Scenario: Agent entry with all optional properties
- **WHEN** `.mason/config.json` declares `"claude": { "package": "@clawmasons/claude-code", "home": "~/projects/claude-config", "mode": "terminal", "role": "writer" }`
- **THEN** the CLI SHALL parse all four fields without error
- **AND** `home`, `mode`, and `role` SHALL be accessible when launching the `claude` agent
...
- **AND** `home`, `mode`, and `role` SHALL all be `undefined`
