...
After `pam install` scaffolds the Docker Compose stack, users must manually run `docker compose -f <path>/docker-compose.yml up` and remember the correct file path. `pam run` and `pam stop` commands are the final step in the agent lifecycle, giving users a single CLI to go from install to running agent without touching Docker directly. These commands also enforce that required `.env` credentials are set before launching.

## What Changes

- Add `pam run <agent> [--runtime=X]` command that starts the Docker Compose stack for an installed agent
- Add `pam stop <agent>` command that tears down the Docker Compose stack
