...
### Requirement: pam list command is registered as a CLI command
...
### Requirement: pam list displays all installed agents as a tree

When `pam list` is run, the command SHALL:
1. Discover packages in the workspace via `discoverPackages()`
2. Find all packages with `pam.type === "agent"`
...
- **WHEN** `pam list` is run in a workspace with one agent that has 2 roles
...
- **WHEN** `pam list` is run in a workspace with multiple agents
- **THEN** each agent SHALL be listed with its full dependency tree

#### Scenario: Empty workspace
- **WHEN** `pam list` is run in a workspace with no agent packages
- **THEN** the command SHALL print "No agents found." and exit with code 1

### Requirement: pam list supports JSON output

#### Scenario: JSON output
- **WHEN** `pam list` is run with `--json`
