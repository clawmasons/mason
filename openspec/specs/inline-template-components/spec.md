...
Defines the inline template components for the note-taker template. All chapter components (apps, tasks, skills) are defined directly within the template directory using `{{projectScope}}` placeholders, making initialized projects fully self-contained.
