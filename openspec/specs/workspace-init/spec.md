...
The `pam init` command that scaffolds a new pam monorepo workspace. Creates the directory structure, npm workspace configuration, pam config files, environment template, and .gitignore. Handles edge cases like existing files and idempotent re-runs.
