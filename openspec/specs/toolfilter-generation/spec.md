...
- **WHEN** agent has one role `role-issue-manager` with `permissions: { "@clawmasons/app-github": { allow: ["create_issue", "list_repos", "add_label"] } }`
- **THEN** `computeToolFilters()` returns a toolFilter for `@clawmasons/app-github` with `mode: "allow"` and `list: ["create_issue", "list_repos", "add_label"]`

#### Scenario: Multiple roles reference the same app
- **WHEN** agent has `role-issue-manager` allowing `["create_issue", "list_repos", "add_label"]` on `@clawmasons/app-github` and `role-pr-reviewer` allowing `["list_repos", "get_pr", "create_review"]` on `@clawmasons/app-github`
- **THEN** `computeToolFilters()` returns a toolFilter for `@clawmasons/app-github` with `list` containing exactly `["create_issue", "list_repos", "add_label", "get_pr", "create_review"]` (union, no duplicates)

#### Scenario: App referenced by one role but not another
- **WHEN** agent has `role-issue-manager` with permissions for `@clawmasons/app-github` and `@clawmasons/app-slack`, and `role-pr-reviewer` with permissions for `@clawmasons/app-github` only
...
- **WHEN** `@clawmasons/app-github` exposes tools `["create_issue", "list_repos", "add_label", "delete_repo", "transfer_repo"]` but the only role allowing github tools permits `["create_issue", "list_repos", "add_label"]`
...
The system SHALL derive a short name from the app's package name for use as the mcpServers key. The short name is computed by stripping the npm scope (e.g., `@clawmasons/`) and the `app-` prefix if present.

#### Scenario: Scoped package with app- prefix
- **WHEN** the app package name is `@clawmasons/app-github`
- **THEN** the short name is `github`

#### Scenario: Scoped package without app- prefix
- **WHEN** the app package name is `@clawmasons/slack-server`
...
- **THEN** the result is a Map where each key is a full app package name (e.g., `@clawmasons/app-github`) and each value has `{ mode: "allow", list: string[] }`
