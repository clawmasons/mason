...
### Requirement: CLI registration for pam add
The `pam add` command SHALL be registered as a Commander.js command with a required `<pkg>` argument and optional variadic `[npmArgs...]` for forwarding to npm.
...
The `pam add` command SHALL delegate package installation to npm by executing `npm install <pkg> [npmArgs...]` in the workspace root directory using `child_process.execFileSync`.

#### Scenario: Successful npm install
- **WHEN** `pam add @clawforge/app-github` is run in a valid workspace
...
- **WHEN** `pam add @clawforge/app-github --save-dev --legacy-peer-deps` is run
...
### Requirement: Post-install pam field validation
After successful npm install, the command SHALL read the installed package's `package.json` and validate its `pam` field using `parsePamField()`.

#### Scenario: Valid pam package
- **WHEN** npm install succeeds and the package has a valid `pam` field
- **THEN** the command SHALL print a success message containing the package name and its pam type
- **AND** SHALL exit with code 0

#### Scenario: Package missing pam field
- **WHEN** npm install succeeds but the package has no `pam` field
- **THEN** the command SHALL run `npm uninstall <pkg>` to clean up
- **AND** SHALL print an error message indicating the package is not a valid pam package
- **AND** SHALL exit with code 1

#### Scenario: Package with invalid pam field
- **WHEN** npm install succeeds but the package's `pam` field fails Zod validation
- **THEN** the command SHALL run `npm uninstall <pkg>` to clean up
...
- **WHEN** `pam add @clawforge/app-github` is run
