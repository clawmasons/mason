...
Commander.js-based CLI entry point for the pam tool. Provides the executable binary, command routing, version/help display, and a modular command registration pattern that all pam CLI commands plug into.

## Requirements

### Requirement: CLI entry point with Commander.js
The system SHALL provide a CLI executable registered as `pam` in package.json's `bin` field, powered by Commander.js with a program name of `pam`, a version matching package.json, and a description of "Packaged Agent Manager — AI agent packaging, governance, and runtime orchestration".

#### Scenario: Display version
- **WHEN** the user runs `pam --version`
- **THEN** the CLI outputs the version from package.json (e.g., `0.1.0`)

#### Scenario: Display help
- **WHEN** the user runs `pam --help`
- **THEN** the CLI outputs usage information listing all registered commands

#### Scenario: Unknown command
- **WHEN** the user runs `pam nonexistent`
...
The system SHALL provide a `bin/pam.js` file with a Node.js shebang (`#!/usr/bin/env node`) that imports and runs the compiled CLI entry point. The package.json `bin` field SHALL map `"pam"` to this file.

#### Scenario: Global install execution
- **WHEN** the package is installed globally via `npm install -g`
- **THEN** running `pam` in a terminal invokes the CLI entry point

#### Scenario: npx execution
- **WHEN** a user runs `npx @clawforge/pam init`
