...
End-to-end integration test that validates the full PAM pipeline: build → pam install → docker compose up → MCP protocol requests via HTTP, ensuring the mcp-proxy works correctly with stdio MCP servers.
