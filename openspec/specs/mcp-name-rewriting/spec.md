# mcp-name-rewriting Specification

## Purpose

Defines the `convertMcpFormat()` utility for rewriting MCP tool name references and the `AgentPackage.mcpNameTemplate` field that declares the naming convention used by each agent runtime.
