...
The `claude-code-agent` runtime SHALL NOT map to any auth variable. Users authenticate by running `/login` inside the container on first run.

#### Scenario: Agent with claude-code-agent runtime has no auth token
- **WHEN** agent has `runtimes: ["claude-code-agent"]`
