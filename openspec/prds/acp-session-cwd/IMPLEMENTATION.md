...
### CHANGE 1: MASON_HOME Utility & chapters.json

Create a shared utility module for resolving `MASON_HOME` and reading/writing `chapters.json`. This is the foundation used by all subsequent changes.

**PRD refs:** REQ-002 (MASON_HOME Environment Variable), PRD §4.1, §4.2

**Summary:** Create `packages/cli/src/runtime/home.ts` — a utility module that: (1) resolves `MASON_HOME` from env var or defaults to `~/.mason`, (2) reads/writes/updates `chapters.json` with type-safe entries for initialized chapter/role combinations, (3) resolves role directories from `chapters.json` (handling `targetDir` overrides), (4) ensures the `MASON_HOME` directory exists with a `.gitignore` ignoring log subdirectories.

**User Story:** As the `init-role` command, I call `getMasonHome()` to get the resolved base directory, then `readChaptersJson()` to check existing entries, and `updateChaptersJson(entry)` to register a new role. As `run-agent`, I call `findRoleEntry(lodge, chapter, role)` to look up where the role's docker-compose lives.
...
function ensureMasonHome(home: string): void;
```

**Testable output:** Unit tests: (a) `getMasonHome()` reads `MASON_HOME` env var, (b) defaults to `~/.mason` when unset, (c) `readChaptersJson` returns empty chapters array when file doesn't exist, (d) `upsertRoleEntry` creates new entry, (e) `upsertRoleEntry` updates existing entry by lodge/chapter/role key, (f) `findRoleEntry` returns matching entry or undefined, (g) `ensureMasonHome` creates directory and `.gitignore`.
...
Create a shared utility for automatically managing `.gitignore` entries when `.mason/` is created in project directories.

**PRD refs:** PRD §4.3 (Per-Project .mason), US-6

**Summary:** Create `packages/cli/src/runtime/gitignore.ts` — utility functions to: (1) check if a directory's `.gitignore` already contains a given pattern, (2) append a pattern to an existing `.gitignore`, (3) orchestrate the "ensure `.mason` is gitignored" logic used by `run-agent`, `run-acp-agent`, and `init-role`. This is a small, focused module used by multiple later changes.
...
Create the new `init-role` command that initializes a host-wide runtime directory for a chapter role at `MASON_HOME/<lodge>/<chapter>/<role>/`.

**PRD refs:** REQ-001 (`chapter init-role`), US-2, US-3, US-7

**Summary:** Create `packages/cli/src/cli/commands/init-role.ts`. The command: (1) reads `MASON_HOME` via the utility from CHANGE 1, (2) discovers packages and resolves the agent/role from the current chapter workspace, (3) determines the role directory (default or `--target-dir`), (4) generates a `docker-compose.yaml` with services for proxy, credential-service, and all agents for the role, (5) backs up existing `docker-compose.yaml` if re-running, (6) updates `chapters.json`, (7) ensures `MASON_HOME/.gitignore` exists. Register in the command index.
...
**User Story:** As an operator, I run `chapter init-role --role writer` from my chapter workspace. It creates `~/.mason/acme/platform/writer/docker-compose.yaml` with services for all agents that use the `writer` role. I can then `run-agent` or `run-acp-agent` from any project directory.

**Testable output:** (a) Creates role directory at `MASON_HOME/<lodge>/<chapter>/<role>/`, (b) generates `docker-compose.yaml` with proxy, credential-service, and agent services, (c) `chapters.json` updated with new entry, (d) `--target-dir` overrides role directory location, (e) re-running backs up existing `docker-compose.yaml`, (f) `MASON_HOME/.gitignore` created if missing, (g) multiple agents for same role all appear in compose.
...
### CHANGE 5: `run-agent` MASON_HOME & Auto-Init

Update `run-agent` to use `MASON_HOME` for role resolution, create per-project `.mason/` for session state, auto-invoke `init-role` when needed, and manage `.gitignore`.

**PRD refs:** REQ-003 (`run-agent` Changes), US-5, US-6

**Summary:** Modify `packages/cli/src/cli/commands/run-agent.ts` to: (1) on invocation, read `MASON_HOME/chapters.json` to find the matching role entry, (2) if not found, automatically run `init-role` logic (from CHANGE 4), (3) use `roleDir` from `chapters.json` (respecting `targetDir` overrides) for docker-build path, (4) create `.mason/` in the current project directory for session-specific state (sessions, logs), (5) use the `.gitignore` utility from CHANGE 2 to ensure `.mason` is ignored in the project.

The key behavioral change: `run-agent` no longer requires a pre-existing `.mason/chapter.json` in the project directory. Instead, it looks up the role from the host-wide `chapters.json` and creates session state locally.

**User Story:** As a developer, I `cd /projects/myapp` and run `chapter run-agent note-taker writer`. Even though I never ran `init-role`, it auto-initializes, creates `~/.mason/acme/platform/writer/`, then starts my agent with `/projects/myapp` mounted as the workspace. `.mason` is added to my project's `.gitignore`.

**Testable output:** (a) Reads role from `chapters.json` when initialized, (b) auto-invokes `init-role` when role not found, (c) creates per-project `.mason/sessions/<id>/` for session state, (d) appends `.mason` to project `.gitignore`, (e) uses `targetDir` from `chapters.json` when set, (f) mounts CWD as `/workspace` (unchanged behavior).
...
Rename the `acp-proxy` command to `run-acp-agent` and add `MASON_HOME` support with auto-init behavior.

**PRD refs:** REQ-004 (Rename `acp-proxy` to `run-acp-agent`), US-5

**Summary:** Rename `packages/cli/src/cli/commands/acp-proxy.ts` to `run-acp-agent.ts`. Update the command registration to use `run-acp-agent` as the command name. Add the same `MASON_HOME` + auto-init logic from CHANGE 5. The old `acp-proxy` registration was already removed in CHANGE 3. Update all imports and references throughout the codebase.
...
**User Story:** As an operator, I run `chapter run-acp-agent --role writer` and it behaves identically to the old `chapter acp-proxy --role writer`, but with the new name, `MASON_HOME` support, and auto-init if the role isn't initialized yet.

**Testable output:** (a) `chapter run-acp-agent --role writer` starts ACP endpoint, (b) auto-inits role if not in `chapters.json`, (c) uses `MASON_HOME` for role resolution, (d) all existing ACP proxy tests pass under new command name, (e) old `acp-proxy` command does not exist.
...
**Summary:** E2E tests refactored to use CLI-only approach with no internal API imports. Test files renamed: `docker-init-full.test.ts` → `build-pipeline.test.ts`, `note-taker-pi.test.ts` → `build-pi-runtime.test.ts`, `acp-proxy.test.ts` → `docker-proxy.test.ts`. All tests use `chapter build` as the sole setup command. Shared helpers extracted to `e2e/tests/helpers.ts` (`copyFixtureWorkspace`, `chapterExec`, `waitForHealth`). Removed internal imports: `validateDockerfiles`, `generateDockerfiles`, `matchServers`, `rewriteMcpConfig`, `extractCredentials`, `generateWarnings`, `openDatabase`, `queryAuditLog`, `logDroppedServers`. ACP unit tests (matcher, rewriter, warnings, audit) removed from e2e — already covered by unit tests in `packages/cli/tests/acp/` and `packages/proxy/tests/hooks/`. vitest.config.ts `resolve.alias` block removed (no longer needed).

**User Story:** As a developer running the test suite, `npx vitest run e2e/` passes with all existing coverage preserved. E2E tests only invoke the `chapter` CLI binary and verify its outputs (exit codes, stdout, generated files, running containers).

**Testable output:** (a) `build-pipeline.test.ts` uses `chapter build` as setup with no internal imports, (b) `build-pi-runtime.test.ts` uses `chapter build` with no `generateDockerfiles` import, (c) `docker-proxy.test.ts` contains only Docker e2e tests (7 tests, no unit tests), (d) zero `../../packages/` imports in e2e/tests/, (e) full E2E suite passes: 44 passed, 2 skipped.

**Implemented** -- [Archived change](../../changes/archive/2026-03-10-e2e-test-updates/) ([proposal](../../changes/archive/2026-03-10-e2e-test-updates/proposal.md), [design](../../changes/archive/2026-03-10-e2e-test-updates/design.md), [tasks](../../changes/archive/2026-03-10-e2e-test-updates/tasks.md)). Source: `e2e/tests/build-pipeline.test.ts`, `e2e/tests/build-pi-runtime.test.ts`, `e2e/tests/docker-proxy.test.ts`, `e2e/tests/helpers.ts`. 1034 unit tests + 44 e2e tests passing.

---

### CHANGE 9: `run-acp-agent` Help Instructions

Add detailed help text to `run-acp-agent` explaining CWD behavior, `.mason` creation, `.gitignore` management, and ACP client configuration examples.

**PRD refs:** REQ-010 (Help Instructions in `run-acp-agent`)

**Summary:** Update the `run-acp-agent` command description and add a `--help` epilog that explains: (1) it will create `.mason/` in the session's CWD for logs, (2) it will append `.mason` to the project's `.gitignore` if present, (3) how to configure `MASON_HOME`, (4) example ACP client configuration JSON for Zed/JetBrains.

**Testable output:** (a) `chapter run-acp-agent --help` displays CWD behavior explanation, (b) help includes `.gitignore` notice, (c) help includes `MASON_HOME` documentation, (d) help includes ACP client config example.
