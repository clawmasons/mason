...
- **Not representative of real usage:** The example is an npm workspace with local packages. Real users will `npm install @clawmasons/forge` into a new directory and expect it to work. There is no `forge init` flow that demonstrates this.
...
### User Goals
- Run `npm install @clawmasons/forge` in an empty directory and have a working CLI.
- Run `forge init --template note-taker` and get a complete, working agent project that demonstrates the forge pattern.
- Use pre-built components from `@clawmasons/forge-core` (apps, tasks, skills, roles) as building blocks in their own agent projects.
...
- Establish `@clawmasons/forge-core` as the standard library of reusable forge components.
...
- The following sequence completes without errors using local tgz packages:
  ...
  cd <forge-repo>
  npm run build
  npm pack                          # → clawmasons-forge-0.1.0.tgz
  cd forge-core && npm pack         # → clawmasons-forge-core-0.1.0.tgz

  # Test in a fresh directory
  mkdir /tmp/test-forge && cd /tmp/test-forge
  npm install <path-to>/clawmasons-forge-0.1.0.tgz
  npx forge init --template note-taker
  # (forge init installs @clawmasons/forge-core from local tgz or path)
...
**US-2:** As an agent builder, I want to install `@clawmasons/forge-core` and reference its components (e.g., `@clawmasons/app-filesystem`) in my own roles and tasks, so that I don't have to rebuild common MCP integrations.
...
|---------|------|-------------|
| `@clawmasons/forge` | Root (`./`) | CLI tool, resolver, proxy, generators |
| `@clawmasons/forge-core` | `forge-core/` | Pre-built library of apps, tasks, skills, roles, agents |
...
`@clawmasons/forge-core` is an npm package structured like a user's forge workspace. It contains pre-built forge components organized in the standard directory layout:
...
├── apps/
│   └── filesystem/
│       └── package.json      # @clawmasons/app-filesystem
├── tasks/
│   └── take-notes/
│       ├── package.json      # @clawmasons/task-take-notes
│       └── prompts/
│           └── take-notes.md
├── skills/
│   └── markdown-conventions/
│       ├── package.json      # @clawmasons/skill-markdown-conventions
│       └── SKILL.md
├── roles/
│   └── writer/
│       └── package.json      # @clawmasons/role-writer
└── agents/
    └── note-taker/
        └── package.json      # @clawmasons/agent-note-taker
```

Each sub-component has its own `package.json` with a unique `name` (e.g., `@clawmasons/app-filesystem`) and a `forge` field. These names are used by forge's resolver for cross-package references. No npm-level `dependencies` exist between sub-components — references are purely forge-field references resolved by forge's discovery system.
...
- Given a user creates a local role that references `@clawmasons/app-filesystem` in its permissions, when `resolveAgent()` runs, then the app resolves from within forge-core.
- Given a user creates a local `apps/filesystem/` package also named `@clawmasons/app-filesystem`, when discovery runs, then the local version takes precedence over forge-core's version.
...
Acceptance criteria:
- Given `@clawmasons/forge-core` is installed in `node_modules`, when `discoverPackages()` runs, then `@clawmasons/app-filesystem`, `@clawmasons/task-take-notes`, `@clawmasons/skill-markdown-conventions`, `@clawmasons/role-writer`, and `@clawmasons/agent-note-taker` are all discovered.
...
- Given a local `apps/filesystem/` package named `@clawmasons/app-filesystem` exists AND `@clawmasons/forge-core` also contains `@clawmasons/app-filesystem`, when discovery runs, then the local version is in the map (workspace precedence preserved).

**REQ-004: Templates Directory**

Template directories are bundled inside the `@clawmasons/forge` package. Each template is a subdirectory of `templates/` containing files that `forge init` copies to the target directory.

```
templates/
└── note-taker/
    ├── package.json          # Pre-configured with @clawmasons/forge-core dependency
    ├── agents/
    │   └── note-taker/
...
- Given the template's `package.json`, when read, then it lists `@clawmasons/forge-core` as a dependency.
- Given the template's agent `package.json`, when read, then it references a local role, which in turn references `@clawmasons/task-take-notes`, `@clawmasons/skill-markdown-conventions`, and `@clawmasons/app-filesystem`.
...
After copying template files and generating the scaffold, `forge init` runs `npm install` to install dependencies (including `@clawmasons/forge-core`).
...
Acceptance criteria:
- Given `forge init --template note-taker --name @acme/my-agent`, when run in an empty directory, then: (a) template files are copied, (b) `package.json` has `name: "@acme/my-agent"`, (c) `.forge/` directory is created, (d) `npm install` runs, (e) `node_modules/@clawmasons/forge-core` exists.
...
- Given `@clawmasons/forge-core` is referenced as a local `.tgz` in the template's `package.json`, when `npm install` runs, then the package installs correctly and discovery finds all components.

**REQ-006: Remove `example/` Directory**

The `example/` directory is removed from the repository. Its contents are migrated to:
- Component definitions → `forge-core/` (with `@clawmasons/*` naming)
...
The new Dockerfile:
- Copies the installed `@clawmasons/forge` package (already compiled) from the workspace
...
- Instead, copy the installed forge package from `node_modules/@clawmasons/forge/`
...
The root `package.json` for `@clawmasons/forge` includes `templates/` in its `files` array so templates are included when the package is published/packed.

The `forge-core/package.json` includes all component directories in its `files` array.

Acceptance criteria:
- Given `npm pack` is run for `@clawmasons/forge`, when the resulting `.tgz` is inspected, then `templates/note-taker/` is included alongside `dist/` and `bin/`.
- Given `npm pack` is run for `@clawmasons/forge-core`, when the resulting `.tgz` is inspected, then `apps/`, `tasks/`, `skills/`, `roles/`, `agents/` directories and their contents are included.
...
│           └── writer/
│               └── package.json
├── forge-core/                       # @clawmasons/forge-core (workspace member)
│   ├── package.json
│   ├── apps/
│   │   └── filesystem/
│   ├── tasks/
...
```
                     ...
                     ┌───────────┴───────────┐
                     ▼                       ▼
              @clawmasons/forge        @clawmasons/forge-core
...
forge init [--template <name>] [--name <project-name>]
  ...
  │
  ├─6─ Run npm install
  │    └── Installs @clawmasons/forge-core and other template deps
  │
...
The `forge install` command copies the pre-built `@clawmasons/forge` from `node_modules` (which already has `dist/`, `bin/`, `node_modules/`, `package.json`) into the `forge/` build context directory. No source, no tsconfig, no compilation.
...
─────────────────────────────────          ───────────────────────────────
(in /tmp/test-forge/)                      (in node_modules/@clawmasons/forge-core/)

agents/note-taker/                         agents/note-taker/
  name: @test-forge/agent-note-taker         name: @clawmasons/agent-note-taker
  └── roles: [@test-forge/role-writer]       └── roles: [@clawmasons/role-writer]
  (local override — takes precedence)             (shadowed by local version)

roles/writer/                              roles/writer/
  name: @test-forge/role-writer              name: @clawmasons/role-writer
  └── tasks: [@clawmasons/task-take-notes]    └── (same references)
  └── skills: [@clawmasons/skill-md-conv]
  └── permissions:                           tasks/take-notes/
       @clawmasons/app-filesystem: {...}      skills/markdown-conventions/
                                             apps/filesystem/
  (User customizes this locally)               (Reusable building blocks)
```

The template provides local agent + role definitions scoped to the project name (e.g., `@test-forge/*`). These shadow the identically-structured components in forge-core. The building blocks (apps, tasks, skills) come from forge-core via npm and keep their `@clawmasons/*` names.
...
| Q2 | Should template names support scope-like syntax (e.g., `forge init --template @clawmasons/note-taker`) for future third-party templates? | Engineering | No |
| Q3 | Should the note-taker template's local agent/role use the project name in their package names (via placeholder substitution), or use fixed `@clawmasons/*` names? | Product | Yes |
