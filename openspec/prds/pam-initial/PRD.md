# Packaged Agent Manager (pam) — Product Requirements Document
...
The Packaged Agent Manager (pam) is a thin wrapper around npm that adds AI agent semantics to the existing npm ecosystem. Every agent component — apps, tasks, skills, roles, and agents themselves — is a standard npm package extended with a `pam` metadata field. This gives pam the full power of npm's dependency resolution, versioning, workspaces, and registry infrastructure while layering on the runtime scaffolding, permission governance, and multi-runtime support that AI agent deployments require.

pam solves three problems simultaneously:
...
- **Runtime Portability:** Agents declare supported runtimes (Claude Code, Codex, Aider, etc.). pam materializes the correct workspace configuration for each, all backed by the same proxy infrastructure.

---

## 2. Design Principles

- **npm-native:** Every pam package is a valid npm package. pam delegates all dependency resolution, versioning, and registry operations to npm.
- **Governance as code:** Permissions are declared in package metadata. pam generates a tbxark/mcp-proxy `config.json` with `toolFilter` entries computed from role permissions — not by trust in the runtime.
...
- **Declarative over imperative:** Agent definitions describe what an agent can do, not how to wire it up. pam generates all runtime plumbing from declarations.
...
pam defines five package types. Each is a standard npm package whose `package.json` includes a `pam` field declaring its type and type-specific metadata.
...
- **composite:** Chains other tasks sequentially or in parallel. Defined by a `tasks` array in the pam field with optional `parallel: true`.
...
An agent is the top-level deployable unit. It declares the roles the agent operates with, the runtimes it supports, and the resources it will access. This is the package that `pam install` targets.
...
| Field | Type | Description |
|-------|------|-------------|
| `runtimes` | string[] | Supported runtime environments. pam scaffolds a workspace directory for each. |
...
pam supports a monorepo layout using npm workspaces. All agent components live in a single repository, organized by type, and are independently publishable.

```
my-agent-project/
├── package.json            # root workspace config
├── pam.config.json         # pam-specific build config
├── apps/
│   ├── github/
│   │   └── package.json    # pam.type = "app"
│   └── slack/
│       └── package.json
├── tasks/
│   ├── triage-issue/
│   │   └── package.json    # pam.type = "task"
│   └── assign-issue/
│       └── package.json
├── skills/
│   └── labeling/
│       └── package.json    # pam.type = "skill"
├── roles/
│   └── issue-manager/
│       └── package.json    # pam.type = "role"
└── agents/
    └── repo-ops/
        └── package.json    # pam.type = "agent"
```
...
The pam CLI wraps npm for package operations and adds agent-specific commands for building, installing, running, and validating agents.

### 5.1 Command Reference

| Command | Description |
|---------|-------------|
| `pam init` | Initializes a pam workspace. Runs `npm init`, creates `.pam/` directory, scaffolds `pam.config.json`. |
| `pam add <pkg>` | Wraps `npm install`. Validates the package has a `pam` field and compatible type. |
| `pam remove <pkg>` | Wraps `npm uninstall`. Checks for dependent packages before removing. |
| `pam build <agent>` | Walks the dependency graph, validates requirements, produces `pam.lock.json`. |
| `pam install <agent-pkg>` | `npm install` + resolves pam graph + scaffolds runtime directory with mcp-proxy config and workspace dirs. |
| `pam run <agent> [--runtime=X]` | Starts Docker Compose stack. If `--runtime` specified, only that runtime starts. Proxy always starts. |
| `pam stop <agent>` | Stops and tears down the Docker Compose stack. |
| `pam list` | Lists installed agents and their resolved role/task/app tree. |
| `pam permissions <agent>` | Displays the resolved permission matrix: role → app → allowed tools, and the generated toolFilter. |
| `pam validate <agent>` | Validates agent graph: checks all task requirements covered by role permissions. |
| `pam publish` | Wraps `npm publish`. Adds pre-publish validation of the `pam` field. |
...
my-workspace/
├── .pam/
│   ├── config.json       # workspace-level pam configuration
│   └── .env.example      # template for credential bindings
├── node_modules/
├── agents/               # runtime directories (created by pam install)
└── package.json
```
...
- **Tool existence:** Every tool in a role's `permissions.allow` must exist in the corresponding app's `pam.tools` list.
...
When `pam install` scaffolds an agent, it generates a self-contained directory with a single tbxark/mcp-proxy Docker container that aggregates all app MCP servers, plus one workspace directory per declared runtime. The proxy is the only infrastructure container — app servers run inside it.
...
├── pam.lock.json                 # resolved dependency graph
│
├── mcp-proxy/
...
A single runtime container supports all roles declared by the agent. Since tbxark/mcp-proxy's `toolFilter` is configured per-`mcpServer` entry (not per-request), pam computes the `toolFilter` as the **union** of all tools allowed across all roles for each app. Per-role scoping is then enforced by the runtime's context layer: the `AGENTS.md` file, slash commands, and task prompts explicitly declare which tools each role may use, and the LLM respects these boundaries.
...
**Security note:** The proxy-layer `toolFilter` guarantees that even if the LLM ignores the soft boundary, it cannot access tools outside the union of all roles. For agents where strict per-role isolation is critical, pam supports an alternative mode: one mcp-proxy instance per role, each with its own `config.json` and `toolFilter`. See section 6.5.
...
services:
  ...
  claude-code:
    ...
    environment:
      ...
      - PAM_ROLES=issue-manager,pr-reviewer
    depends_on:
      - mcp-proxy
    stdin_open: true
    tty: true
...
The `mcp-proxy/config.json` is the heart of the runtime. pam generates it by walking the agent's roles, collecting all referenced apps, and computing the `toolFilter` for each app as the union of all role allow-lists. The config follows the tbxark/mcp-proxy schema:
...
For each app referenced by any role in the agent, pam computes the `toolFilter` as follows:
...
4. Validate that every tool in the union exists in the app's `pam.tools` declaration.
...
tbxark/mcp-proxy supports `authTokens` for bearer-token authentication. pam generates a random `PAM_PROXY_TOKEN` at install time and configures both the proxy and the runtime settings to use it. This prevents unauthorized access to the proxy endpoint from other containers or processes on the network.
...
- **Environment file:** The generated `.env` file maps environment variables consumed by mcp-proxy's env interpolation (`${VAR}` syntax). `pam install` prompts for values or reads from `PAM_ENV_FILE`.
- **Secrets manager:** The `.pam/config.json` can specify a secrets provider (AWS Secrets Manager, Vault). `pam run` resolves secrets at startup and writes them to `.env`.
- **Credential sidecar:** For token lifecycle management (refresh, rotation), apps can declare a `credentialProvider` in their pam field. A sidecar process handles rotation and updates the proxy's environment.

### 6.5 Strict Per-Role Isolation (Alternative Mode)

For agents where the soft runtime-layer boundary is insufficient and hard per-role tool isolation is required, pam supports an alternative scaffolding mode: one mcp-proxy instance per role.

```
# pam install --strict-roles @clawforge/agent-repo-ops
...
A materializer is a plugin that translates the abstract pam dependency graph into a specific runtime's native configuration format. Materializers are registered with pam and invoked during `pam install` for each runtime declared by the agent.
...
Claude Code sees a single MCP server (`pam-proxy`) that exposes all tools from all apps, already filtered by the proxy's `toolFilter`. The runtime's permission system allows all proxy tools, since the proxy itself is the enforcement layer.
...
# Generated by pam from @clawforge/task-triage-issue@0.3.1
...
You are an agent managed by pam (Packaged Agent Manager).
...
Third-party materializers are registered via pam plugins — an npm package with `pam.type` of `"materializer"`:

```json
{
  "name": "@my-org/pam-materializer-aider",
  "version": "1.0.0",
  "pam": { "type": "materializer", "runtime": "aider" },
...
The following sequence describes what happens when a user runs `pam install <agent-package>`:

1. **npm install:** Delegates to npm to install the agent package and all transitive dependencies into `node_modules/`.
2. **Graph resolution:** Reads the agent's `pam` field, walks the typed dependency graph (roles → tasks → apps + skills), produces a `ResolvedAgent` with all metadata flattened.
3. **Validation:** Runs the same checks as `pam validate`. Install aborts with actionable errors on failure.
4. **Compute toolFilters:** For each app, unions the allow-lists across all roles to produce the `toolFilter`. Validates all tools exist in the app's `pam.tools`.
...
8. **Write pam.lock.json:** Snapshots the resolved graph with exact versions and generated file hashes.
...
pam enforces the ClawForge governance thesis: AI agents should operate under least-privilege access, with permissions declared in code and enforced at the infrastructure level.
...
2. For each app, resolve the app's `pam.tools` list.
...
tbxark/mcp-proxy supports `logEnabled` per server and at the proxy level. When enabled, the proxy logs tool registration and invocation events. pam sets `logEnabled: true` on both `mcpProxy` and all `mcpServers` by default. Logs are written to the container's stdout and captured by Docker's logging driver.

For structured audit trails, pam generates a log configuration with rotation:
...
`pam validate` integrates into CI/CD pipelines as a governance gate. A PR changing role permissions or task requirements triggers validation. If the configuration violates the permission model, the CI check fails. The generated `toolFilter` can be diffed in code review to audit permission changes.

---

## 10. Registry Strategy

- **Public registry (npmjs.com):** Open-source community packages published under the `@clawforge` scope. Apps, skills, and tasks that are generally useful.
...
npm's `.npmrc` scoping supports this natively — different scopes can resolve to different registries without any pam-level configuration.
...
- **Hot-reload:** `pam watch` mode that regenerates the mcp-proxy `config.json` and restarts the proxy when source packages change.
...
- **Per-request role enforcement:** A custom proxy extension or middleware that reads the `X-PAM-Role` header and applies per-role `toolFilter` dynamically, eliminating the soft boundary layer entirely.

---

## Appendix A: pam Field JSON Schema Reference
...
| `mcpServers.<name>.options.toolFilter.mode` | per-server | `allow` or `block`. pam always generates `allow`. |
