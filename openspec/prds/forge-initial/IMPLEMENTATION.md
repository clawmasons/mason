# Implementation Plan — Agent Forge System (forge)

**PRD:** [openspec/prds/forge-initial/PRD.md](./PRD.md)
**Version:** 0.1.0
**Date:** March 2026
...
Set up the TypeScript project with build tooling (tsconfig, vitest, eslint) and define the core `forge` field types and Zod schema validators for all five package types (app, skill, task, role, agent). This is the type foundation everything else builds on.
...
## CHANGE: forge init — Workspace Scaffolding ✅

Implement the CLI entry point (`bin/forge`) using Commander.js and the `forge init` command. Creates the foundational workspace structure: root package.json with workspaces config, `.forge/` directory with config.json, `.env.example` template, and the type-organized directory layout (apps/, tasks/, skills/, roles/, agents/).

**References:** PRD §5.1 (Command Reference — forge init), PRD §5.2, PRD §4 (Monorepo Structure)

**User Story:** As an agent developer, I can run `forge init` in an empty directory and get a ready-to-use monorepo workspace where I can start creating agent components, so I don't have to manually set up boilerplate.

**Testable output:**
- `forge init` in a temp directory creates the expected file tree matching PRD §5.2
- Root package.json has `"workspaces": ["apps/*", "tasks/*", "skills/*", "roles/*", "agents/*"]`
- `.forge/config.json` and `.env.example` exist with reasonable defaults
- Running `forge init` twice warns that workspace already exists

**Implemented:** 2026-03-03
- [Proposal](../../changes/archive/2026-03-03-forge-init-workspace-scaffolding/proposal.md)
- [Design](../../changes/archive/2026-03-03-forge-init-workspace-scaffolding/design.md)
- [Tasks](../../changes/archive/2026-03-03-forge-init-workspace-scaffolding/tasks.md)
- [Specs: cli-framework](../../changes/archive/2026-03-03-forge-init-workspace-scaffolding/specs/cli-framework/spec.md)
- [Specs: workspace-init](../../changes/archive/2026-03-03-forge-init-workspace-scaffolding/specs/workspace-init/spec.md)
...
Implement the graph resolver that reads installed npm packages, parses their `forge` fields, and walks the typed dependency graph (agent → roles → tasks → apps + skills) to produce a `ResolvedAgent` data structure. This is the core engine that powers validate, install, and build.

**References:** PRD §3.1 (Dependency Graph), PRD §8 step 2 (Graph resolution)

**User Story:** As forge internals, given an agent package name, I can resolve its complete dependency tree — all roles, their tasks, each task's required apps and skills — into a single flattened `ResolvedAgent` object, so that downstream commands (validate, install, build) can operate on a stable, fully-resolved representation.
...
## CHANGE: forge validate — Graph Validation ✅

Implement `forge validate <agent>` which runs all validation checks against a resolved agent graph. This is the governance gate that CI/CD pipelines use to catch permission and configuration errors before deployment.

**References:** PRD §5.3 (forge validate), PRD §9.4 (Validation as Governance Gate)

**User Story:** As an agent developer, I can run `forge validate my-agent` and get a pass/fail result with detailed error messages, so I catch permission gaps, missing tools, and misconfigured apps before attempting to install or run anything.

**Testable output:**
- Validates requirement coverage: task's required app tools are in parent role's allow-list
- Validates tool existence: role's allow-list tools exist in app's `forge.tools`
...
**Implemented:** 2026-03-03
- [Proposal](../../changes/archive/2026-03-03-forge-validate-graph-validation/proposal.md)
- [Design](../../changes/archive/2026-03-03-forge-validate-graph-validation/design.md)
- [Tasks](../../changes/archive/2026-03-03-forge-validate-graph-validation/tasks.md)
- [Specs: graph-validation](../../changes/archive/2026-03-03-forge-validate-graph-validation/specs/graph-validation/spec.md)
...
**User Story:** As forge install, I can generate a complete mcp-proxy config.json from a resolved agent, so that the proxy enforces the hard governance boundary — only tools explicitly allowed by at least one role are accessible.
...
- Uses `${FORGE_PROXY_TOKEN}` placeholder for proxy auth (actual token generated by `forge install`)
...
**User Story:** As a Claude Code user, after `forge install`, I want a workspace directory that Claude Code natively understands — with my agent's roles described in AGENTS.md, each task as a slash command scoped to the right role's tools, and skill files available for context — so I can start operating the agent immediately.

**Testable output:**
- Generates `.claude/settings.json` with single forge-proxy MCP server entry matching PRD §7.2.1
...
Implement the Docker Compose generator that assembles the `docker-compose.yml` with the mcp-proxy service, one service per declared runtime, the agent-net bridge network, and the `.env` file template. Also generates `forge.lock.json` with the resolved graph snapshot.
...
- `forge.lock.json` contains resolved graph with exact versions
...
## CHANGE: forge install — Full Orchestrated Flow ✅

Wire together all preceding components into the `forge install <agent-pkg>` command: npm install → graph resolution → validation → toolFilter computation → proxy config generation → runtime materialization → compose generation → lock file → credential prompting. This is the primary user-facing command.
...
**Implemented:** 2026-03-03
- [Proposal](../../changes/archive/2026-03-03-forge-install-orchestrated-flow/proposal.md)
...
## CHANGE: forge build, list, and permissions Commands ✅

Implement the remaining read-only CLI commands: `forge build <agent>` (resolves graph + produces forge.lock.json without scaffolding), `forge list` (displays installed agents and their role/task/app tree), `forge permissions <agent>` (displays the resolved permission matrix and generated toolFilter).

**References:** PRD §5.1 (Command Reference — build, list, permissions)

**User Story:** As an agent developer, I can run `forge list` to see what's installed, `forge permissions my-agent` to audit exactly which tools each role can access, and `forge build` to lock the dependency graph — so I have full visibility into the agent's configuration before running it.

**Testable output:**
- `forge build` produces forge.lock.json with resolved versions and dependency tree
- `forge list` outputs a tree view: agent → roles → tasks → apps/skills
- `forge permissions` outputs a matrix: role → app → [allowed tools], plus the generated toolFilter
- All commands return non-zero exit codes when no agent is installed or agent not found

**Implemented:** 2026-03-03
- [Proposal](../../changes/archive/2026-03-03-forge-build-list-permissions/proposal.md)
- [Design](../../changes/archive/2026-03-03-forge-build-list-permissions/design.md)
- [Tasks](../../changes/archive/2026-03-03-forge-build-list-permissions/tasks.md)
- [Specs: build-command](../../changes/archive/2026-03-03-forge-build-list-permissions/specs/build-command/spec.md)
- [Specs: list-command](../../changes/archive/2026-03-03-forge-build-list-permissions/specs/list-command/spec.md)
- [Specs: permissions-command](../../changes/archive/2026-03-03-forge-build-list-permissions/specs/permissions-command/spec.md)
...
## CHANGE: forge run & stop — Docker Lifecycle ✅

Implement `forge run <agent> [--runtime=X]` and `forge stop <agent>`. Run starts the Docker Compose stack (or a single runtime if `--runtime` specified). Stop tears it down. Both delegate to `docker compose` under the hood.

**References:** PRD §5.1 (Command Reference — run, stop), PRD §6.2 (Docker Compose Orchestration)

**User Story:** As an agent operator, I can run `forge run repo-ops` to bring up the proxy and all runtime containers, then `forge stop repo-ops` to tear everything down — so I manage the full agent lifecycle through forge without touching Docker directly.
...
**Implemented:** 2026-03-03
- [Proposal](../../changes/archive/2026-03-03-forge-run-stop-docker-lifecycle/proposal.md)
- [Design](../../changes/archive/2026-03-03-forge-run-stop-docker-lifecycle/design.md)
- [Tasks](../../changes/archive/2026-03-03-forge-run-stop-docker-lifecycle/tasks.md)
- [Specs: run-command](../../changes/archive/2026-03-03-forge-run-stop-docker-lifecycle/specs/run-command/spec.md)
- [Specs: stop-command](../../changes/archive/2026-03-03-forge-run-stop-docker-lifecycle/specs/stop-command/spec.md)
- [Main Spec: run-command](../../specs/run-command/spec.md)
- [Main Spec: stop-command](../../specs/stop-command/spec.md)

---

## CHANGE: forge add & remove — Dependency Management ✅

Implement `forge add <pkg>` (wraps `npm install` with forge field validation and type compatibility checking) and `forge remove <pkg>` (wraps `npm uninstall` with dependent package checking).
...
- `forge add` with a non-forge package (no `forge` field) rejects with error
- `forge remove` with dependents warns and requires `--force` to proceed
- `forge remove` without dependents runs npm uninstall cleanly

**Implemented:** 2026-03-03
- [Proposal](../../changes/archive/2026-03-03-forge-add-remove/proposal.md)
- [Design](../../changes/archive/2026-03-03-forge-add-remove/design.md)
- [Tasks](../../changes/archive/2026-03-03-forge-add-remove/tasks.md)
- [Specs: add-command](../../changes/archive/2026-03-03-forge-add-remove/specs/add-command/spec.md)
- [Specs: remove-command](../../changes/archive/2026-03-03-forge-add-remove/specs/remove-command/spec.md)
- [Main Spec: add-command](../../specs/add-command/spec.md)
- [Main Spec: remove-command](../../specs/remove-command/spec.md)

---

## CHANGE: forge publish — Registry Publishing

Implement `forge publish` which wraps `npm publish` with pre-publish validation of the `forge` field. Ensures the package is well-formed before it hits the registry.

**References:** PRD §5.1 (Command Reference — publish), PRD §10 (Registry Strategy)

**User Story:** As a package author, I can run `forge publish` and know that my package's forge metadata is validated before it reaches the registry — so consumers can trust that published forge packages are well-formed.

**Testable output:**
- `forge publish --dry-run` validates the forge field and reports pass/fail without publishing
- Invalid forge field blocks publish with clear error
- Valid forge field delegates to `npm publish` with all args forwarded
...
**User Story:** As an agent developer targeting multiple runtimes, I want `forge install` to also generate a Codex-compatible workspace alongside Claude Code — so the same agent definition works across runtimes without manual configuration.
...
Implement the `--strict-roles` flag for `forge install` that scaffolds one mcp-proxy instance per role, each with its own config.json and toolFilter restricted to exactly that role's allow-list. Updates docker-compose generation to include multiple proxy services.
...
**Testable output:**
- `forge install --strict-roles` generates separate `mcp-proxy-{role-name}/config.json` per role
