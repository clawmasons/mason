...
### CHANGE 1: Rename CLI Binary from `chapter` to `mason`

Rename the CLI binary, program name, and all internal references from `chapter` to `mason`. This is the foundational change that all subsequent work builds on.

**PRD refs:** REQ-001 (Rename CLI Binary)

**Summary:** Change the bin entry in `packages/cli/package.json` from `chapter` to `mason`, update the Commander program name in `packages/cli/src/cli/index.ts`, rename `bin/chapter.js` to `bin/mason.js`, update the proxy Dockerfile entrypoint in `packages/cli/src/generator/proxy-dockerfile.ts`, and update all E2E test helpers and references. This is a pure rename — no command restructuring yet.

**User Story:** As a developer, I want to type `mason --help` and see the CLI, so the binary matches the product name. All existing commands still work as top-level commands (restructuring happens in CHANGE 2).

**Scope:**
- `packages/cli/package.json` — bin entry rename
- `packages/cli/src/cli/index.ts` — program name
- `bin/chapter.js` → `bin/mason.js`
...
**Testable output:** `npx mason --help` works. All existing E2E tests pass with the new binary name. `npx chapter` no longer resolves.
...
**User Story:** As a user, I run `mason chapter build` instead of `chapter build`, and `mason acp --role writer` instead of `chapter run-acp-agent --role writer`. The old command names are gone.
...
- Update E2E tests to use new command paths (e.g., `mason chapter build`)

**Testable output:** `mason --help` shows `init`, `agent`, `acp`, `chapter`. `mason chapter --help` shows all subcommands. `mason agent <agent> <role>` works. `mason acp --role <role>` works. Old names (`run-agent`, `run-acp-agent`) are rejected. All E2E tests pass.
...
**User Story:** As a chapter author with `"baseImage": "node:22-bookworm"` and `"mounts": [{ "source": "${LODGE_HOME}", "target": "/home/mason/${LODGE}" }]` in my role, when I run `mason chapter build`, the generated Dockerfile uses `FROM node:22-bookworm` with apt packages installed, and the compose file includes the extra volume mount.
...
**Testable output:** Unit tests verify: (a) Dockerfile with custom baseImage uses correct FROM, (b) Dockerfile with aptPackages includes apt-get install, (c) Dockerfile without these fields is unchanged, (d) compose volumes include role mounts with env vars resolved, (e) readonly mounts append `:ro`. Integration: `mason chapter build` on a chapter with role mounts produces correct Docker artifacts.

**Implemented:** [spec](../../changes/archive/2026-03-10-agent-dockerfile-mounts/spec.md)

---

### CHANGE 5: `mason init` — Lodge Initialization Command

Implement the `mason init` command that creates a lodge directory structure with CHARTER.md and config.json registry.

**PRD refs:** REQ-003 (Lodge Initialization), REQ-009 (CHARTER.md Template)

**Summary:** Create `packages/cli/src/cli/commands/lodge-init.ts` implementing the `mason init` command. Create the CHARTER.md template at `packages/cli/templates/charter/CHARTER.md`. The command resolves `MASON_HOME`, `LODGE`, and `LODGE_HOME` from CLI flags or env vars with documented defaults. It creates the lodge directory, copies CHARTER.md, creates `chapters/`, and registers the lodge in `MASON_HOME/config.json`. Idempotent — skips if lodge already exists with a `chapters/` directory. Also add lodge config helpers to `packages/cli/src/runtime/home.ts`.

**User Story:** As a new user, I run `mason init --lodge myproject` and get `~/.mason/myproject/` with `CHARTER.md` and `chapters/` directory, plus a registry entry in `~/.mason/config.json`.
...
**Testable output:** Unit tests verify: (a) creates MASON_HOME and config.json from scratch, (b) creates LODGE_HOME with CHARTER.md and chapters/, (c) registers lodge in config.json, (d) idempotent — skips if already initialized, (e) does not overwrite existing CHARTER.md, (f) custom --lodge-home is registered correctly. CLI test: `mason init --lodge test` creates expected directory structure.
...
**User Story:** As the bootstrap flow, when `mason chapter init --name acme.initiate --template initiate` runs, it scaffolds a complete chapter workspace with the chapter-creator role ready to analyze projects and create new chapters.
...
**Testable output:** `mason chapter init --name test.initiate --template initiate` creates a valid workspace. `mason chapter build` succeeds on the scaffolded workspace (generates Dockerfiles with `node:22-bookworm` base, apt packages, and lodge mount). Template placeholders are correctly substituted.

**Implemented:** [spec](../../changes/archive/2026-03-10-initiate-chapter-template/spec.md)

---

### CHANGE 7: `mason acp --chapter initiate` — Bootstrap Flow

Add `--chapter` and `--init-agent` options to the `mason acp` command. When `--chapter initiate` is specified, run the full bootstrap flow: lodge init → chapter init → chapter build → standard ACP startup.
...
**User Story (US-1):** As a new user, I run `npx mason acp --chapter initiate --role chapter-creator` and it bootstraps everything — lodge, initiate chapter, docker containers — and starts accepting ACP client connections. Zero prior setup required.
...
**Testable output:** Integration test: `mason acp --chapter initiate --role chapter-creator` on a clean system creates lodge, chapter, builds, and starts ACP endpoint. Idempotent test: running again skips init/build. Unit test: bootstrap logic calls init and build in correct order with correct CWD.
...
Create an end-to-end test that starts the `mason acp` agent from an ACP client (like `acpx`) and verifies the full lifecycle: ACP client spawns the agent process, connects, creates a session, and the agent responds.

**PRD refs:** REQ-004 (Bootstrap Flow acceptance criteria), US-1 (Single-command ACP setup)

**Summary:** Create an E2E test that simulates what an ACP client like `acpx` or Zed does: spawn `mason acp --role <role>` as a child process (stdio transport), send ACP protocol messages (`initialize`, `session/new` with a CWD), verify the agent starts and responds to requests. Test both the standard flow (existing chapter workspace) and optionally the bootstrap flow (`--chapter initiate`). This validates the entire pipeline from ACP client perspective — the same code path that `acpx` would exercise.

**User Story:** As a developer configuring `acpx` or Zed to use mason, I want confidence that `npx mason acp --role writer` works end-to-end when spawned as a subprocess with stdio transport — the same way ACP clients launch agent servers.
...
- Tests:
  1. **Spawn and initialize**: Spawn `mason acp --role writer` via stdio, send ACP `initialize` message, verify `initialized` response
...
**Testable output:** E2E test suite passes: ACP client can spawn mason, establish a session, list tools, and tear down — validating the full ACP client integration path.
...
**Summary:** Create `packages/placeholders/` directory (outside monorepo workspaces) with placeholder packages for: `clawmasons`, `@clawmasons/acp`, `@clawmasons/mcp-proxy`, `mason-ai`, `mason-com`, `clawmason`, and org-scoped names. Each has a minimal `package.json` and `README.md` directing users to the official project. Create `scripts/publish-placeholders.sh` to publish them all.

**User Story (US-8):** As a security-conscious publisher, I want placeholder packages on npm for all mason-adjacent names so attackers cannot publish malicious packages under those names.
...
### CHANGE 11: `mason acp` Help Text with ACP Client Config Example

Extend the `--help` output for `mason acp` to include a complete ACP client configuration example.
...
**User Story:** As a developer setting up an ACP client, I run `mason acp --help` and see a ready-to-copy JSON config block I can paste into my client settings.

**Scope:**
- Modified: `packages/cli/src/cli/commands/run-acp-agent.ts` — extended help epilog

**Testable output:** `mason acp --help` output includes the ACP client config JSON example with `agent_servers`, env vars, and usage instructions.
