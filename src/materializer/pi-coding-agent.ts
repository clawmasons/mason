 ...
 * 1. Registers the chapter MCP proxy as a tool provider
 * 2. Registers each chapter task as a pi command
 */
function generateExtensionIndexTs(
  ...
  const proxyUrl = `${proxyEndpoint}${pathSuffix}`;

  const bearerValue = proxyToken
    ? `Bearer ${proxyToken}`
    : "Bearer \\${process.env.CHAPTER_PROXY_TOKEN}";

  const lines: string[] = [];
  lines.push("// Generated by the pi-coding-agent materializer");
  lines.push("export default (pi) => {");
  lines.push("  // Connect to chapter proxy for tools");
  lines.push("  pi.registerMcpServer({");
  lines.push('    name: "chapter",');
  lines.push(`    transport: "${proxyType}",`);
  lines.push(`    url: "${proxyUrl}",`);
  lines.push("    headers: {");
  lines.push(`      Authorization: "${bearerValue}"`);
  lines.push("    }");
  lines.push("  });");
...
export const piCodingAgentMaterializer: RuntimeMaterializer = {
  ...
  materializeWorkspace(
    ...
    // .pi/extensions/chapter-mcp/index.ts — MCP proxy + task commands
    result.set(
      ".pi/extensions/chapter-mcp/index.ts",
      generateExtensionIndexTs(member, proxyEndpoint, proxyToken),
    );
