...
function displayCompletionInstructions(agents: ResolvedAgent[]): void {
  ...
  if (first) {
    ...
    console.log(`    clawmasons run-agent ${first.agentShort} ${first.roleShort}`);
    ...
    if (examples.length > 1) {
      ...
      for (let i = 1; i < examples.length; i++) {
        const ex = examples[i];
        if (ex) {
          console.log(`    clawmasons run-agent ${ex.agentShort} ${ex.roleShort}`);
        }
      }
    }

    console.log("\n  Configure an ACP client:");
    console.log(`    clawmasons run-acp-agent --role ${first.roleShort}`);
