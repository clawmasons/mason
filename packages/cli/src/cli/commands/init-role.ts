...
export function generateInitRoleComposeYml(opts: {
  ...
  const { dockerBuildPath, agents, role, roleShortName } = opts;
  ...
  let yaml = `# Generated by chapter init-role
services:
  ${proxyServiceName}:
    build:
...
export async function initRole(
  ...
  try {
    ...
    if (!fs.existsSync(dockerBuildPath)) {
      throw new Error(
        `Docker build directory not found at ${dockerBuildPath}. Run "chapter build" first.`,
      );
    }
