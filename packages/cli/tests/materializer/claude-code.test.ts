  ...
  describe("materializeWorkspace", () => {
    describe(".claude.json", () => {
      it("generates single mason entry with default SSE proxy", () => {
        ...
        expect(mcp.mcpServers.mason.url).toBe("http://mcp-proxy:9090/sse");
      ...
      it("generates single mason entry with custom port", () => {
        ...
        expect(mcp.mcpServers.mason.url).toBe("http://mcp-proxy:8080/sse");
      });
      ...
      it('generates mason entry with http transport (streamable-http maps to "http" for Claude Code)', () => {
        ...
        const mcp = JSON.parse(result.get(".claude.json")!);
        expect(mcp.mcpServers.mason.type).toBe("http");
        expect(mcp.mcpServers.mason.url).toBe("http://mcp-proxy:9090/mcp");
      });
      ...
      it("bakes actual token into auth header when proxyToken provided", () => {
        ...
        const mcp = JSON.parse(result.get(".claude.json")!);
        expect(mcp.mcpServers.mason.headers.Authorization).toBe("Bearer abc123def456");
      });
    ...
    describe("slash commands", () => {
      ...
      it("uses task name as command filename under scope directory", () => {
        const agent = makeRepoOpsAgent();
        const result = claudeCodeMaterializer.materializeWorkspace(agent, "http://mcp-proxy:9090");

        // @clawmasons/task-triage-issue -> @clawmasons/task-triage-issue.md
        expect(result.has(".claude/commands/@clawmasons/task-triage-issue.md")).toBe(true);
      });
      ...
      it("generates empty content when no prompt defined", () => {
        const agent = makeRepoOpsAgent();
        agent.roles[1].tasks[0].prompt = undefined;
        const result = claudeCodeMaterializer.materializeWorkspace(agent, "http://mcp-proxy:9090");

        const reviewCmd = result.get(".claude/commands/@clawmasons/task-review-pr.md")!;
        expect(reviewCmd).toBe("");
      });

      it("includes frontmatter when task has metadata fields", () => {
        const agent = makeRepoOpsAgent();
        agent.roles[0].tasks[0] = {
          ...agent.roles[0].tasks[0],
          displayName: "Triage Issue",
        ...
        const triageCmd = result.get(".claude/commands/@clawmasons/task-triage-issue.md")!;
        expect(triageCmd).toContain("---");
        expect(triageCmd).toContain("name: Triage Issue");
        expect(triageCmd).toContain("description: Triage incoming issues");
        expect(triageCmd).toContain("category: ops");
      });
    });
